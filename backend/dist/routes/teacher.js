...
const TEACHER_ONLY = (0, auth_1.requireRole)("teacher");
const ownsClass = async (teacherId, classId) => {
    const [rows] = await db_1.pool.query("SELECT 1 FROM teacher_classes WHERE teacher_id = ? AND class_id = ?", [teacherId, classId]);
    return rows.length > 0;
};
/* ============================================================
   GET CLASSES FOR TEACHER
   ============================================================ */
router.get("/classes", auth_1.authMiddleware, TEACHER_ONLY, (0, asyncHandler_1.asyncHandler)(async (req, res) => {
...
/* ============================================================
   ...
   ============================================================ */
router.get("/attendance/report", auth_1.authMiddleware, TEACHER_ONLY, (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    const [rows] = await db_1.pool.query(`SELECT
       ...
       ORDER BY a.date ASC, s.first_name ASC, s.surname ASC`, [classId, startDate, endDate]);
    ...
    res.setHeader("Content-Disposition", `attachment; filename="attendance_report_class${classId}_${startDate}_to_${endDate}.csv"`);
    res.send(csv);
}));
/* ============================================================
   ...
   ============================================================ */
router.get("/attendance/:classId", auth_1.authMiddleware, TEACHER_ONLY, (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    const [rows] = await db_1.pool.query(`SELECT
       ...
       LEFT JOIN attendance a
        ...
        AND a.date = ?
       WHERE sc.class_id = ?
       ORDER BY s.first_name, s.surname`, [classId, date, classId]);
    res.json({ students: rows, date });
}));
/* ============================================================
   GET ATTENDANCE HISTORY (PAST DATES) FOR A CLASS
   ============================================================ */
router.get("/attendance/:classId/history", auth_1.authMiddleware, TEACHER_ONLY, (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const classId = Number(req.params.classId);
    if (!(await ownsClass(req.user.userId, classId))) {
        return res.status(403).json({ message: "You are not assigned to this class" });
    }
    const [rows] = await db_1.pool.query(`SELECT
       ...
       WHERE a.class_id = ?
       GROUP BY a.date
       ORDER BY a.date DESC`, [classId]);
    res.json({ history: rows });
}));
/* ============================================================
   MARK ATTENDANCE (defaults to today; pass date to backfill a past day)
   ============================================================ */
router.post("/attendance/mark", auth_1.authMiddleware, TEACHER_ONLY, (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    await db_1.pool.query(`INSERT INTO attendance (class_id, student_id, date, status)
       VALUES (?, ?, ?, ?)
       ON DUPLICATE KEY UPDATE status = VALUES(status)`, [classId, studentId, date, normalizedStatus]);
...
/* ============================================================
   ...
   ============================================================ */
router.post("/tasks/create", auth_1.authMiddleware, TEACHER_ONLY, (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (studentId) {
        if (!(await (0, featureFlags_1.isFeatureEnabled)("student_notes", req.user.schoolId))) {
            return res.status(403).json({ message: "Independent homework is currently disabled" });
        }
        const [studentRows] = await db_1.pool.query("SELECT 1 FROM student_classes WHERE class_id = ? AND student_id = ?", [classId, studentId]);
        if (studentRows.length === 0) {
            return res.status(400).json({ message: "Student is not in this class" });
        }
        resolvedStudentId = Number(studentId);
    }
    await db_1.pool.query(`INSERT INTO tasks (class_id, student_id, title, description, due_date)
       VALUES (?, ?, ?, ?, ?)`, [classId, resolvedStudentId, title, description, due_date]);
    res.json({ message: "Task created" });
}));
/* ============================================================
   DELETE TASK (NEW)
   ============================================================ */
router.delete("/tasks/:id", auth_1.authMiddleware, TEACHER_ONLY, (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const taskId = req.params.id;
    const [rows] = await db_1.pool.query(`SELECT t.id
       FROM tasks t
       JOIN teacher_classes tc ON tc.class_id = t.class_id
       WHERE t.id = ? AND tc.teacher_id = ?`, [taskId, req.user.userId]);
    if (rows.length === 0) {
        return res.status(403).json({ message: "You are not assigned to this task's class" });
    }
    await db_1.pool.query("DELETE FROM tasks WHERE id = ?", [taskId]);
...
const logParentContactView = async (entry) => {
    await db_1.pool.query(`INSERT INTO parent_contact_view_log
       (teacher_user_id, student_id, class_id, school_id)
     VALUES (?, ?, ?, ?)`, [entry.teacherUserId, entry.studentId, entry.classId, entry.schoolId]);
};
/* ============================================================
   ...
   ============================================================ */
router.get("/classes/:classId/students/:studentId/parent-contacts", auth_1.authMiddleware, TEACHER_ONLY, (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const classId = Number(req.params.classId);
    ...
    const [guardianRows] = await db_1.pool.query(`SELECT
         p.first_name,
         p.middle_name,
         p.surname,
       ...
       WHERE sg.student_id = ? AND sg.status = 'approved'`, [studentId]);
...
/* ============================================================
   ...
   ============================================================ */
router.get("/tasks", auth_1.authMiddleware, TEACHER_ONLY, (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const teacherId = req.user.userId;
    const [rows] = await db_1.pool.query(`SELECT
         t.id,
         t.title,
