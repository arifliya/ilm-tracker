...
// creates a class.
const SENDERS = (0, auth_1.requireRole)("admin", "owner", "system_admin");
const isPlatformWide = (req) => req.user.role === "system_admin";
const AUDIENCES = ["parent", "staff"];
...
const audienceRoleNames = (audience) => audience === "parent" ? ["parent"] : ["teacher", "staff", "admin", "owner", "maintainer", "treasurer"];
/* ============================================================
   SEND A NOTIFICATION
   ============================================================ */
router.post("/", auth_1.authMiddleware, SENDERS, (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (!(await (0, featureFlags_1.isFeatureEnabled)("notifications", schoolId))) {
    ...
    const [recipientRows] = await db_1.pool.query(`SELECT u.id
       FROM users u
       JOIN roles r ON r.id = u.role_id
       WHERE u.school_id = ? AND r.name IN (?)`, [schoolId, roleNames]);
    const recipientIds = recipientRows.map(r => r.id);
    const conn = await db_1.pool.getConnection();
    try {
        await conn.beginTransaction();
        const [result] = await conn.query(`INSERT INTO notifications (school_id, sender_user_id, audience, title, message)
         VALUES (?, ?, ?, ?, ?)`, [schoolId, req.user.userId, audience, String(title).trim(), String(message).trim()]);
        const notificationId = result.insertId;
        if (recipientIds.length > 0) {
            const values = recipientIds.map(id => [notificationId, id]);
            await conn.query(`INSERT INTO notification_recipients (notification_id, user_id) VALUES ?`, [values]);
        }
        await conn.commit();
...
/* ============================================================
   ...
   ============================================================ */
router.get("/sent", auth_1.authMiddleware, SENDERS, (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const params = [];
    let where = "";
...
router.get("/", auth_1.authMiddleware, (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    if (!(await (0, featureFlags_1.isFeatureEnabled)("notifications", req.user.schoolId))) {
        return res.json([]);
    }
    const [rows] = await db_1.pool.query(`SELECT
         n.id,
       ...
       LEFT JOIN staff_details sd ON sd.user_id = n.sender_user_id
       WHERE nr.user_id = ?
       ORDER BY n.created_at DESC`, [req.user.userId]);
    res.json(rows);
}));
/* ============================================================
   MARK A NOTIFICATION AS READ
   ============================================================ */
router.post("/:id/read", auth_1.authMiddleware, (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    if (!(await (0, featureFlags_1.isFeatureEnabled)("notifications", req.user.schoolId))) {
        return res.status(403).json({ message: "The notifications feature is currently disabled for this school" });
    }
    const [result] = await db_1.pool.query(`UPDATE notification_recipients
       SET read_at = NOW()
       WHERE notification_id = ? AND user_id = ? AND read_at IS NULL`, [req.params.id, req.user.userId]);
    if (result.affectedRows === 0) {
        const [existing] = await db_1.pool.query(`SELECT 1 FROM notification_recipients WHERE notification_id = ? AND user_id = ?`, [req.params.id, req.user.userId]);
