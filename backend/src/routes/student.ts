...
router.get("/classes", authMiddleware, STUDENT_ONLY, async c => {
  const studentUserId = c.get("user")!.userId;
...
router.get("/tasks", authMiddleware, STUDENT_ONLY, async c => {
  const studentUserId = c.get("user")!.userId;
  ...
  const { rows } = await c.get("db").query(
    `SELECT t.id, t.title, t.description, t.due_date, c.class_name,
     ...
     WHERE s.user_id = $1 AND (t.student_id IS NULL OR t.student_id = s.id)`,
    [studentUserId]
  );
