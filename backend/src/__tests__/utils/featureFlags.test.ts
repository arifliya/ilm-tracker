...
describe("isFeatureEnabled", () => {
  it("returns false when the flag does not exist", async () => {
    ...
    const result = await isFeatureEnabled("unknown_flag", 1);
  ...
  it("uses the school override when one is set (enabled)", async () => {
    mockQuery
      ...
      .mockResolvedValueOnce(rows([{ default_enabled: 0, override_enabled: 1 }]));

    const result = await isFeatureEnabled("notifications", 5);
  ...
  it("uses the school override when one is set (disabled)", async () => {
    mockQuery
      .mockResolvedValueOnce(rows([]))
      .mockResolvedValueOnce(rows([{ default_enabled: 1, override_enabled: 0 }]));
  ...
  it("falls back to default_enabled when there is no override", async () => {
    mockQuery
      .mockResolvedValueOnce(rows([]))
      .mockResolvedValueOnce(rows([{ default_enabled: 1, override_enabled: null }]));

    const result = await isFeatureEnabled("attendance_report", null);
    expect(result).toBe(true);
  });
});
