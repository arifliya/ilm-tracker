...
-- SEQUENCE RESYNC
-- ============================
-- The notifications insert above used explicit ids (1, 2, 3) against an
-- identity column that's GENERATED BY DEFAULT specifically to allow that.
-- Postgres doesn't advance the backing sequence for explicit-value
-- inserts, so without this the next natural INSERT (no id given) would
-- try to reuse id 1 and collide.
